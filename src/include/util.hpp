...
void print_vk_to_file(r1cs_ppzksnark_verification_key<ppT> vk, string pathToFile)
{
  ...
  vk_data << hex << A.X << endl;
  vk_data << hex << A.Y << endl;

  vk_data << hex << B.X << endl;
  vk_data << hex << B.Y << endl;

  vk_data << hex << C.X << endl;
  vk_data << hex << C.Y << endl;

  vk_data << hex << gamma.X << endl;
  vk_data << hex << gamma.Y << endl;

  vk_data << hex << gamma_beta_1.X << endl;
  vk_data << hex << gamma_beta_1.Y << endl;

  vk_data << hex << gamma_beta_2.X << endl;
  vk_data << hex << gamma_beta_2.Y << endl;

  vk_data << hex << Z.X << endl;
  vk_data << hex << Z.Y << endl;

  vk_data << hex << IC_0.X << endl;
  vk_data << hex << IC_0.Y << endl;

  for(size_t i=0; i<IC.size(); i++) {
    G1<ppT> IC_N(IC.rest[i]);
    IC_N.to_affine_coordinates();
    vk_data << hex << IC_N.X << endl;
    vk_data << hex << IC_N.Y << endl;
  }
...
void print_proof_to_file(r1cs_ppzksnark_proof<ppT> proof, string pathToFile)
{
  ...
  proof_data << hex << A_g.X << endl;
  proof_data << hex << A_g.Y << endl;

  proof_data << hex << A_h.X << endl;
  proof_data << hex << A_h.Y << endl;

  proof_data << hex << B_g.X << endl;
  proof_data << hex << B_g.Y << endl;

  proof_data << hex << B_h.X << endl;
  proof_data << hex << B_h.Y << endl;

  proof_data << hex << C_g.X << endl;
  proof_data << hex << C_g.Y << endl;

  proof_data << hex << C_h.X << endl;
  proof_data << hex << C_h.Y << endl;

  proof_data << hex << H.X << endl;
  proof_data << hex << H.Y << endl;

  proof_data << hex << K.X << endl;
  proof_data << hex << K.Y << endl;
